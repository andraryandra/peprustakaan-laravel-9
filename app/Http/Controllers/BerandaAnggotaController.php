...
class BerandaAnggotaController extends Controller
{
    ...
    public function index()
    {
        ...
        $ebook_count = Ebook::where('user_id', $userId)->count();
        $mading_count = Mading::where('user_id', $userId)->count();
        ...
        $ebookChartData = HistoryEbook::select('ebook_id', DB::raw('COUNT(*) as total_access'))
            ->whereHas('ebook', function ($query) use ($userId) {
        ...
        return view('layouts_user.dashboard_anggota', compact(
            "user",
            'users',
            'users_count',
            'users_anggota_count',
