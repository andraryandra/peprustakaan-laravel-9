                    ...
                    <tbody>
                        @foreach ($about as $data)
                            <tr>
                                ...
                                <td class="text-center">{{ $data->teks2 }}</td>
                                <td class="text-end" style="size: 30px;">
                                    <div class="col-md-4 text-end">
                                        <button onclick="editAbout({{$data->id}})" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModalEdit" class="btn btn-primary fw-bold rounded-pill px-4 shadow float-end">
                                            <i class="bi bi-pencil-square"></i>
                                        </button>
                                    </div>
                                    {{-- <div class="col-md-4 text-start">
                                        <form onsubmit="return cofirm('Apakah anda yakin?');" action="{{ url('about.destroy' $data->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button class="btn btn-danger" type="submit">
                                                <a href="/about/{{ $data->id }}" method="post" onsubmit="return confirm('Apakah anda yakin?');">
